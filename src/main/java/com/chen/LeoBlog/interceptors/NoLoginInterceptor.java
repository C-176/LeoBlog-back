...
import com.chen.LeoBlog.utils.InterceptorUtil;
...
public class NoLoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (token == null || StrUtil.isBlank(token)) {
            ...
            InterceptorUtil.responseMsg(response, 401, "用户未登录");
            return false;
        }
        //根据token去redis中查询对应的用户信息
        Object s = redisTemplate.opsForValue().get(RedisConstant.USER_LOGIN + token);
